"""
    All Pegasus code is based on and edited from a combination of the original code contained within the supplementary material for the lightweight-gan paper
    ...
    and lucidrains' implementation of lightweight-gan (https://github.com/lucidrains/lightweight-gan)
    therefore all credit goes to them.
"""
import os
import json
...
class RandomApplyThreeWay(nn.Module):
    def __init__(self, prob, fn1, fn2, fn_else = lambda x: x):
        super().__init__()
        self.fn1 = fn1
        self.fn2 = fn2
        self.fn_else = fn_else
...
class ImageDataset(Dataset):
    def __init__(
        ...
        self.transform = transforms.Compose([
            ...
            RandomApplyThreeWay(aug_prob_rot, transforms.RandomRotation(90), transforms.RandomRotation(270)),
            transforms.ToTensor(),
...
class AugWrapper(nn.Module):
    ...
    def forward(self, images, prob = 0., types = [], detach = False, **kwargs):
        ...
        with context():
            # if random() < prob:
            images = random_hflip(images, prob=0.50)
            images = DiffAugment(images, types=types, prob=prob)
...
# SLE
# Adapted from original codebase from the Lightweight GAN paper.
class SLEoriginal(nn.Module):
    def __init__(self, chan_in, chan_out):
        super().__init__()

        self.main = nn.Sequential(  nn.AdaptiveAvgPool2d(4), 
                                    nn.Conv2d(chan_in, chan_out, 4, 1, 0, bias=False), nn.LeakyReLU(0.1),
                                    nn.Conv2d(chan_out, chan_out, 1, 1, 0, bias=False), nn.Sigmoid() )
    ...
    def __init__(
        ...
        *,
        ...
        for ind in range(num_upsamples):
            last_layer = False # ind == (num_upsamples - 1)
        ...
        self.layers.append(nn.Sequential(
                                    nn.Conv2d(chans, final_chan, 3, 1, 1, bias=False),
                                    nn.Tanh() ))    
...
class LightweightGAN(nn.Module):
    def __init__(
        ...
        *,
        ...
        # For dual discriminator experiments.
        ...
        if optimizer == "adam":
            ...
            self.DH_opt = Adam(self.DH.parameters(), lr = lr * ttur_mult, betas=(0.5, 0.9))
            # self.DB_opt = Adam(self.DB.parameters(), lr = lr * ttur_mult, betas=(0.5, 0.9))
        elif optimizer == "adabelief":
            self.G_opt = AdaBelief(self.G.parameters(), lr = lr, betas=(0.5, 0.9))
            self.DH_opt = AdaBelief(self.DH.parameters(), lr = lr * ttur_mult, betas=(0.5, 0.9))
            # # self.DB_opt = AdaBelief(self.DB.parameters(), lr = lr * ttur_mult, betas=(0.5, 0.9))
        else:
        ...
        self.cuda(rank)
        self.DH_aug = AugWrapper(self.DH, image_size)
        # self.DB_aug = AugWrapper(self.DB, image_size)
...
class Trainer():
    def __init__(
        ...
        **kwargs
        ...
        self.DH_scaler = GradScaler(enabled = self.amp)
        # self.DB_scaler = GradScaler(enabled = self.amp)
    ...
    def init_GAN(self):
        ...
        if self.is_ddp:
            ...
            self.DH_ddp = DDP(self.GAN.DH, **ddp_kwargs)
            self.DH_aug_ddp = DDP(self.GAN.DH_aug, **ddp_kwargs)
            # self.DB_ddp = DDP(self.GAN.DB, **ddp_kwargs)
            # self.DB_aug_ddp = DDP(self.GAN.DB_aug, **ddp_kwargs)
    ...
    def set_data_src(self, folder, loader):
        num_workers = default(self.num_workers, math.ceil(NUM_CORES / self.world_size / 2))
        if loader == "H":
            self.datasetH = ImageDataset(folder, self.image_size, transparent = self.transparent, greyscale = self.greyscale, aug_prob = 0.05, aug_prob_rot=0.02)
            num_samples = len(self.datasetH)
            sampler = DistributedSampler(self.datasetH, rank=self.rank, num_replicas=self.world_size, shuffle=True) if self.is_ddp else None
            dataloader = DataLoader(self.datasetH, num_workers = num_workers, batch_size = math.ceil((self.batch_size / self.world_size )//2), sampler = sampler, shuffle = not self.is_ddp, drop_last = True, pin_memory = True)
            self.loaderH = cycle(dataloader)
        if loader == "B":
            self.datasetB = ImageDataset(folder, self.image_size, transparent = self.transparent, greyscale = self.greyscale, aug_prob = 0.30, aug_prob_rot=0.20)
            num_samples = len(self.datasetB)
            sampler = DistributedSampler(self.datasetB, rank=self.rank, num_replicas=self.world_size, shuffle=True) if self.is_ddp else None
            dataloader = DataLoader(self.datasetB, num_workers = num_workers, batch_size = math.ceil((self.batch_size / self.world_size )//2), sampler = sampler, shuffle = not self.is_ddp, drop_last = True, pin_memory = True)
            self.loaderB = cycle(dataloader)
    ...
    def train(self):
        assert exists(self.loaderH), 'You must first initialize the data source with `.set_data_src(<folder of images>)`'
        assert exists(self.loaderB), 'You must first initialize the data source with `.set_data_src(<folder of images>)`'
        ...
        DH = self.GAN.DH if not self.is_ddp else self.DH_ddp
        # DB = self.GAN.DB if not self.is_ddp else self.DH_ddp
        DH_aug = self.GAN.DH_aug if not self.is_ddp else self.DH_aug_ddp
        # DB_aug = self.GAN.DB_aug if not self.is_ddp else self.DB_aug_ddp
        # print(self.is_ddp)
        ...
        # train discriminator
        self.GAN.DH_opt.zero_grad()
        for i in gradient_accumulate_contexts(self.gradient_accumulate_every, self.is_ddp, ddps=[DH_aug, G]):
            latents = torch.randn(batch_size, latent_dim).cuda(self.rank)
            image_batchH = next(self.loaderH).cuda(self.rank)
            image_batchB = next(self.loaderB).cuda(self.rank)
            # image_batchB.requires_grad_()

            # Merge horses and birds alternately
            # image_batchH = torch.cat([image_batchH, image_batchB], dim=-1).view(-1, image_batchB.shape[-1])
            shape = list(image_batchB.shape)
            shape[0] *= 2
            image_batch = torch.zeros(shape, device=device)
            image_batch[::2, :] = image_batchH   # Index every second row, starting from 0
            image_batch[1::2, :] = image_batchB  # Index every second row, starting from 1 
            image_batch.requires_grad_()


            # torch.cat([a, b], dim=-1).view(-1, a.shape[-1])
            with amp_context():
                with torch.no_grad():
                    generated_images = G(latents)

                fake_output, fake_output_32x32, _ = DH_aug(generated_images, detach = True, **aug_kwargs)
                real_output, real_output_32x32, real_aux_loss = DH_aug(image_batch,  calc_aux_loss = True, **aug_kwargs)
            ...
            if apply_gradient_penalty:
                ...
                outputs = list(map(self.DH_scaler.scale, outputs)) if self.amp else outputs
                ...
                inv_scale = safe_div(1., self.DH_scaler.get_scale()) if self.amp else 1.
            ...
            self.DH_scaler.scale(disc_loss).backward()
            total_disc_loss += divergence

        self.last_recon_loss = aux_loss.item()
        self.d_loss = float(total_disc_loss.item() / self.gradient_accumulate_every)
        self.DH_scaler.step(self.GAN.DH_opt)
        self.DH_scaler.update()

        # For dual discriminator experiments.
        # # train discriminator for birds
        # self.GAN.DB_opt.zero_grad()
        # for i in gradient_accumulate_contexts(self.gradient_accumulate_every, self.is_ddp, ddps=[DB_aug, G]):
        #     latents = torch.randn(batch_size, latent_dim).cuda(self.rank)
        #     image_batchB = next(self.loaderB).cuda(self.rank)
        #     image_batchB.requires_grad_()
        #     # image_batchB = next(self.loaderB).cuda(self.rank)
        ...
        #         fake_output, fake_output_32x32, _ = DB_aug(generated_images, detach = True, **aug_kwargs)
        #         real_output, real_output_32x32, real_aux_loss = DB_aug(image_batchB,  calc_aux_loss = True, **aug_kwargs)

        #         # fake_outputB, fake_output_32x32B, _ = DB_aug(generated_images, detach = True, **aug_kwargs)
        #         # real_outputB, real_output_32x32B, real_aux_lossB = DB_aug(image_batchB,  calc_aux_loss = True, **aug_kwargs)
        ...
        #     if apply_gradient_penalty:
        #         outputs = [real_output, real_output_32x32]
        #         outputs = list(map(self.DB_scaler.scale, outputs)) if self.amp else outputs

        #         scaled_gradients = torch_grad(outputs=outputs, inputs=image_batchB,
        #                                grad_outputs=list(map(lambda t: torch.ones(t.size(), device = image_batchB.device), outputs)),
        #                                create_graph=True, retain_graph=True, only_inputs=True)[0]

        #         inv_scale = safe_div(1., self.DB_scaler.get_scale()) if self.amp else 1.
        ...
        #     disc_loss.register_hook(raise_if_nan)
        #     self.DB_scaler.scale(disc_loss).backward()
        #     total_disc_loss += divergence

        # self.last_recon_loss = aux_loss.item()
        # self.d_loss = float(total_disc_loss.item() / self.gradient_accumulate_every)
        # self.DB_scaler.step(self.GAN.DB_opt)
        # self.DB_scaler.update()
        ...
        for i in gradient_accumulate_contexts(self.gradient_accumulate_every, self.is_ddp, ddps=[DH_aug, G]):
            ...
            with amp_context():
                generated_images = G(latents)
                # For dual discriminator experiments.
                # fake_outputB, fake_output_32x32B, _ = DB_aug(generated_images, **aug_kwargs)
                fake_outputH, fake_output_32x32H, _ = DH_aug(generated_images, **aug_kwargs)
                fake_output_loss = fake_outputH.mean(dim = 1) + fake_output_32x32H.mean(dim = 1)
                # fake_output_loss = (fake_outputH.mean(dim = 1) + fake_output_32x32H.mean(dim = 1))# / 2

                epochs = (self.steps * batch_size * self.gradient_accumulate_every) / ((len(self.datasetH) + len(self.datasetB)) // 2)
    ...
    def save(self, num):
        save_data = {
            ...
            # 'DB_scaler': self.DB_scaler.state_dict(),
            'DH_scaler': self.DH_scaler.state_dict()
        }
    ...
    def load(self, num=-1, print_version=True):
        ...
        if 'DH_scaler' in load_data:
            self.DH_scaler.load_state_dict(load_data['DH_scaler'])
        # if 'DB_scaler' in load_data:
        #     self.DB_scaler.load_state_dict(load_data['DB_scaler'])
